 ...
 * 
 * Changes:
 * - Added case in physx-body createGeometry() to handle planes
 * - Added linear/angular lock flags configuration to physx-body
 */
...
AFRAME.registerComponent('physx-body', {
  ...
  schema: {
    ...
    linearLockFlags: {type: 'array', default: []},

    angularLockFlags: {type: 'array', default: []}
  },
  events: {
    ...
    object3dset: function(e) {
      if (this.rigidBody) {
        ...
        if (type === 'dynamic') {
          let lockFlagValues = 0;
          component.data.linearLockFlags.forEach(flag => {
            const FLAG = flag.toUpperCase();
            switch (FLAG) {
              case 'X':
              case 'Y':
              case 'Z':
                lockFlagValues |= PhysX.PxRigidDynamicLockFlag[`eLOCK_LINEAR_${FLAG}`].value;
                break;
              default:
                console.error(`Invalid linear lock flag: ${flag}. Valid options are x, y, and z.`)
            }
          });
          component.data.angularLockFlags.forEach(flag => {
            const FLAG = flag.toUpperCase();
            switch (FLAG) {
              case 'X':
              case 'Y':
              case 'Z':
                lockFlagValues |= PhysX.PxRigidDynamicLockFlag[`eLOCK_ANGULAR_${FLAG}`].value;
                break;
              default:
                console.error(`Invalid angular lock flag: ${flag}. Valid options are x, y, and z.`)
            }
          });
          if (lockFlagValues !== 0) {
            const lockFlags = new PhysX.PxRigidDynamicLockFlags(lockFlagValues);
  ...
  createGeometry(o) {
    if (o.el.hasAttribute('geometry'))
    {
      ...
      switch(geometry.primitive)
      {
        ...
        case 'plane':
          // PxPlaneGeometry normal by default is always +x up, so just use a really thin box here
          // createConvexMeshGeometry gives planes too much volume
          return new PhysX.PxBoxGeometry(geometry.width / 2, geometry.height / 2, 0.01);
        default:
...
AFRAME.registerComponent('physx-joint', {
  ...
  schema: {
    ...
    // Joint anchor pose
    anchorPosition: {type: 'vec3', default: {x: 0, y: 0, z: 0}},

    anchorRotation: {type: 'vec4', default: {w: 1, x: 0, y: 0, z: 0}}
  },
  events: {
  ...
  async createJoint() {
    ...
    this.update();
    // this.joint.setLocalPose(PhysX.PxJointActorIndex.eACTOR0.value, {
    //   translation: {
    //     x: this.data.anchorPosition.x,
    //     y: this.data.anchorPosition.y,
    //     z: this.data.anchorPosition.z,
    //   },
    //   rotation: {
    //     w: this.data.anchorRotation.w,
    //     x: this.data.anchorRotation.x,
    //     y: this.data.anchorRotation.y,
    //     z: this.data.anchorRotation.z,
    //   }
    // });
    // this.joint.setLocalPose(PhysX.PxJointActorIndex.eACTOR1.value, {
    //   translation: {
    //     x: this.data.anchorPosition.x,
    //     y: this.data.anchorPosition.y,
    //     z: this.data.anchorPosition.z,
    ...
    //   }
    // });
    // console.log(this.joint.getLocalPose(PhysX.PxJointActorIndex.eACTOR0.value).translation.y);
    // console.log(this.joint.getLocalPose(PhysX.PxJointActorIndex.eACTOR1.value).translation.y);
    // console.log(this.joint.getRelativeTransform());
  ...
  events: {
    stateadded: function(e) {
      if (e.detail === 'grabbed')
      {
        ...
        let el = Util.resolveGrabRedirection(e.target);
        ...
        Util.callLater(() => {
          ...
          Util.positionObject3DAtTarget(el.object3D, manipulator.endPoint)
